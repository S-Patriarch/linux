...
//------------------------------------------------------------------------------
static bool t_shouldstop {false}; // флаг остановки потока
static std::string FILE_LOGGER {"slog"};
//------------------------------------------------------------------------------
static struct {
   std::string _strlogger {""}; // строка записи
   bool _iswritelogger {false}; // флаг возможности записи
} log;
//------------------------------------------------------------------------------
...
{ 
   ...
   // проверяем на необходимость остановки потока
      if (::t_shouldstop) break;
      else {
         // проверяем на необходимость записи
         if (log._iswritelogger) {
            l.write(log._strlogger);
            log._strlogger = "";
            log._iswritelogger = false;
         }
      }
   }
   return NULL;
...
std::int32_t main(std::int32_t argc, char** argv)
{
   ...
   pthread_t tid;
   tcp.tcp_pthread_create(&tid,NULL,::write_to_log_file,NULL);
